  ...
  return (
    <div className="space-y-4">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-sm text-muted-foreground mt-1">{t('conflicts.subtitle')}</p>
      </motion.div>

      <WidgetCard title={t('conflicts.map')} className="[&>div:last-child]:p-0 [&>div:last-child]:h-[350px]">
        ...
        {conflicts.map((c, i) => (
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-2 mb-3 text-xs">
              <div><span className="text-muted-foreground">{t('conflicts.region')}:</span> <span className="text-foreground">{c.region}</span></div>
              <div><span className="text-muted-foreground">{t('conflicts.status')}:</span> <span className="text-foreground">{c.status}</span></div>
              <div><span className="text-muted-foreground">{t('conflicts.parties')}:</span> <span className="text-foreground">{c.parties.join(', ')}</span></div>
              <div><span className="text-muted-foreground">{t('conflicts.casualties')}:</span> <span className="text-destructive font-mono">{c.casualties}</span></div>
            </div>
            <div className="border-t border-border/30 pt-3">
              <h4 className="text-[11px] text-muted-foreground font-medium mb-2">{t('conflicts.recent_events')}</h4>
              <div className="space-y-2">
                {c.timeline.map((ev, j) => (
                  <div key={j} className="flex gap-2 text-[11px]">
                    <span className="text-muted-foreground font-mono shrink-0">{ev.date}</span>
